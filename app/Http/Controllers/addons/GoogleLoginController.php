...
class GoogleLoginController extends Controller
{
    ...
    private function configDriver($type, $login_type,$slug)
    {
    ...
    {
        $vendor = User::where('slug',session()->get('slug'))->where('is_available',1)->where('is_deleted',2)->first();
        ...
        try {
            ...
            if (session()->get('logintype') == "user") {
                if ($request->logintype == "google") {
                    ...
                    $googleuserdata = $this->configDriver('user', 'google',session()->get('slug'))->stateless()->user();
                    ...
                    if (!empty($checkuser)) {
                        ...
                        if (Auth::user()->type == 3) {
                            if (Auth::user()->is_available == 1) {
                                ...
                                $getcartlist = Cart::where('vendor_id', $vendor->id)->where('user_id', Auth::user()->id)->get();
                                if ($getcartlist->count() > 0) {
                                    return redirect(session()->get('slug') . '/heckout')->with('sucess', trans('messages.success'));
                                } else {
                        ...
                        if (Auth::user()->type == 3) {
                            if (Auth::user()->is_available == 1) {
                                ...
                                if ($getcartlist->count() > 0) {
                                    return redirect(session()->get('slug') . '/checkout')->with('sucess', trans('messages.success'));
                                } else {
                                    return redirect('/' . session()->get('slug'))->with('sucess', trans('messages.success'));
                                }
                            } else {
            ...
            if (session()->get('logintype') == "vendor") {
                if ($request->logintype == "google") {
                    $googleuserdata = $this->configDriver('vendor', 'google',session()->get('slug'))->stateless()->user();
