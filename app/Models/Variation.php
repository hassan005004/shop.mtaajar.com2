...

{
    ...
    protected $fillable=['product_id','name','price','qty','original_price','min_order','max_order','low_qty','stck_management','is_available'];
    public static function variant_name($name = '', $counter = 0, $product_id = 0)
    {
        ...
        $retuen_name['product_id'] = 'verians['.$counter.'][product_id]';
        $retuen_name['product_id_val'] = 0;
        ...
        if(!empty($name)) {
            $ProductVariantOption = Variation::where('name', $name)->where('product_id', $product_id)->first();
            if(!empty($ProductVariantOption)) {
                ...
                $retuen_name['product_id_val'] = $ProductVariantOption->product_id;
    ...
    public function getproductbyvariantId($id){
        ...
        return Products::find($id)->variants_json;
    }
}