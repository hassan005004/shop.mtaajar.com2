...
<div class="col ">
    <a href="{{ URL::to(@$vendordata->slug . '/products/' . $getproductdata->slug) }}">
        ...
        <div
            class="card rounded-4 h-100 overflow-hidden shadow-theme-9 {{ session()->get('direction') == 2 ? 'rtl' : '' }}">
            ...
            <div class="card-img position-relative">
                ...
                @if ($off > 0)
                    <div class="sale-label-9 {{ session()->get('direction') == 2 ? 'rtl' : ' ' }}">
                        ...
                        @if ($getproductdata->has_variation == 1)
                            <a href="{{ URL::to(@$vendordata->slug . '/products/' . $getproductdata->slug) }}"
                                ...
                                <div class="product-cart-button w-100">
                                    ...
                                    <p class="px-2 fs-7 fw-500 text-capitalize">{{ trans('labels.add_to_cart') }}
                                    </p>
                                </div>
                            </a>
                        @else
                ...
                <!-- options -->
                <ul
                    class="option-wrap justify-content-center align-items-center d-grid product_icon2 px-2 w-auto {{ session()->get('direction') == 2 ? 'rtl' : ' ' }}">
                    @if (@helper::checkaddons('customer_login'))
                        ...
                        @endif
                    @endif
                    <li data-tooltip="{{ trans('labels.view') }}" class="m-0 tooltip-right">
                    ...
                    @if (@helper::checkaddons('product_reviews'))
                        @if (helper::appdata($vendordata->id)->product_ratting_switch == 1)
                            <p class="d-flex fs-8 product-star align-items-center">
                                ...
                                <span
                                    class="text-lightslategray fs-8 fw-500">{{ number_format($getproductdata->ratings_average, 1) }}</span>
                            </p>
                        @endif
                    @endif
                </div>
