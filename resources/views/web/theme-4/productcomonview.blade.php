    ...
    <div class="card h-100 w-100 border">
        ...
        <div class="card-img position-relative">
            <a href="{{ URL::to(@$vendordata->slug . '/products/' . $getproductdata->slug) }}">
                <img src="{{ $getproductdata['product_image']->image_url }}" class="products-img w-100 img-1"
                    alt="product image">
            ...
            <ul class="option-wrap">
                ...
                <li tooltip="View" class="rounded-circle">
                    <a class="circle-round wishlist-btn" onclick="productview('{{ $getproductdata->id }}','0')">
        ...
        @if ($off > 0)
            <span
                class="{{ session()->get('direction') == 2 ? 'sale-label-off' : 'sale-label-on' }}">{{ $off }}%
        ...
        <div class="card-body pb-0">
            ...
            <a href="{{ URL::to(@$vendordata->slug . '/products/' . $getproductdata->slug) }}">
                <p class="m-0 text-dark product-name line-2">{{ $getproductdata->name }}</p>
            </a>
            ...
            <div>
                <h5 class="text-dark fs-7 fw-bold product-price text-truncate">
                    ...
                    @if ($original_price > $price)
                        <del
                            class="text-muted fs-8 fw-600 d-block mt-1">{{ helper::currency_formate($original_price, $getproductdata->vendor_id) }}</del>
                    @endif
