...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="text-capitalize fw-600 text-dark fs-4">{{ trans('labels.media') }} <span> ({{ $media->count() }}
                {{ trans('labels.images') }})
            </span></h5>
    </div>
    <div class="row">
        <form action=" {{ URL::to('admin/media/add_image') }}" method="post" enctype="multipart/form-data"
            ...
            <div class="modal-footer">
                <button type="submit" class="btn px-sm-4 btn-primary">{{ trans('labels.save') }}</button>
            </div>
        </form>
        <div class="col-12">
                                        ...
                                        <div class="one-img">
                                            <a href="{{ @helper::image_path($data->image) }}">
                                                <img src="{{ @helper::image_path($data->image) }}" alt="pro img"
                                                    ...
                                                    id="{{ @helper::image_path($data->image) }}">
                                            </a>
                                        </div>
                                        <div class="dropdown">
                                            ...
                                            <ul
                                                class="dropdown-menu {{ session()->get('direction') == 2 ? 'rtl' : 'ltr' }}">
                                                <li><a class="dropdown-item {{ Auth::user()->type == 4 ? (helper::check_access('role_import_product', Auth::user()->role_id, $vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}"
