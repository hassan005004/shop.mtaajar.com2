...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <nav aria-label="breadcrumb">
        ...
        <ol class="breadcrumb m-0">
            ...
            <li class="breadcrumb-item"><a href="{{ URL::to('admin/custom_domain') }}">{{ trans('labels.custom_domains') }}</a></li>
            ...
            <div class="card-body">
                ...
                <form class="col-md-12 my-2" action="{{ URL::to('admin/custom_domain/save') }}">
                    ...
                    <div class="form-group {{session()->get('direction') == 2 ? 'text-start' : 'text-end'}}">
                        <a href="{{ URL::to('admin/custom_domain') }}" class="btn btn-outline-danger px-sm-4">{{ trans('labels.cancel') }}</a>
                        <button @if (env('Environment')=='sendbox' ) type="button" onclick="myFunction()" @else type="submit" @endif class="btn btn-primary px-sm-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_custom_domains', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">{{ trans('labels.save') }}</button>
                    </div>
