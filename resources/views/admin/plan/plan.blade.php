...
@section('content')
    <div class="d-flex justify-content-between align-items-center">
        <h5 class="text-capitalize fw-600 text-dark fs-4">{{ trans('labels.pricing_plan') }}</h5>
        @if (Auth::user()->type == '1')
            <a href="{{ URL::to('admin/plan/add') }}" class="btn btn-secondary px-sm-4 d-flex"> <i
                    class="fa-regular fa-plus mx-1"></i>{{ trans('labels.add') }} </a>
        @endif
    </div>

    <div class="row g-3 pt-4 {{ Auth::user()->type == 1 ? 'sort_menu' : '' }}" id="carddetails"
