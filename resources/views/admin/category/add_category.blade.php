                        ...
                        <form action="{{URL::to('admin/categories/save')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group {{session()->get('direction') == 2 ? 'text-start' : 'text-end'}}">
                                    ...
                                    <button class="btn btn-primary px-sm-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}" @if(env('Environment')=='sendbox') type="button" onclick="myFunction()" @else type="submit" @endif>{{ trans('labels.save') }}</button>
