            ...
            <div class="card-body">
                <form method="post" action="{{ URL::to('admin/systemaddons/store') }}" name="about" id="about" enctype="multipart/form-data">
                    ...
                    <div class="{{session()->get('direction') == 2 ? 'text-start' : 'text-end'}}">
                        ...
                        <button type="button" class="btn btn-secondary px-sm-4" onclick="myFunction()">{{ trans('labels.install') }}</button>
                        @else
                        <button type="submit" class="btn btn-secondary px-sm-4">{{ trans('labels.install') }}</button>
                        @endif
                    </div>
                </form>
            </div>
