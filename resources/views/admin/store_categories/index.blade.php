...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ URL::to('admin/store_categories/add') }}"
            class="btn btn-secondary px-sm-4 text-capitalize d-flex {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">
                                        ...
                                        <td>
                                            @if ($category->is_available == '1')
                                                <a tooltip="{{ trans('labels.active') }}"
                                                    ...
                                                    class="btn btn-sm hov btn-outline-success {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                            ...
                                            @else
                                                <a tooltip="{{ trans('labels.inactive') }}"
                                                    ...
                                                    class="btn btn-sm hov btn-outline-danger {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                <a href="{{ URL::to('admin/store_categories/edit-' . $category->id) }}"
                                                    ...
                                                    class="btn btn-info hov btn-sm {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                ...
                                                <a tooltip="{{ trans('labels.delete') }}"
                                                    ...
                                                    class="btn btn-danger hov btn-sm {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}">
