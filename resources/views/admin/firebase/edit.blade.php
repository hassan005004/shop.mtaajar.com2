                    ...
                    <div class="card-body">
                        <form action="{{URL::to('/admin/notification/update-'. $editfirebase->id)}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group {{session()->get('direction') == 2 ? 'text-start' : 'text-end'}}">
                                ...
                                <button @if (env('Environment') == 'sendbox') type="button" onclick="myFunction()" @else type="submit" @endif class="btn btn-primary px-sm-4 {{Auth::user()->type == 4 ? (helper::check_access('role_firebase_notification',Auth::user()->role_id,Auth::user()->vendor_id,'edit') == 1 ? '' : 'd-none') : '' }}">{{ trans('labels.save') }}</button>
                            </div>
