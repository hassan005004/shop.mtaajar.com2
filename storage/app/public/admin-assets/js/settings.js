$(".basicinfo").on("click", function() {
  ...
  if (
    ...
    $(this).attr("data-tab") == "themesettings"
  ) {
    $("html, body").animate(
      {
        scrollTop: 0
...
$("#template_type")
  .on("change", function() {
    ...
    $("#" + $(this).find(":selected").data("attribute"))
      .find("textarea")
      .prop("required", true);
  })
  .change();
...
function add_features(icon, title, description) {
  ...
  var html =
    ...
    title +
    '" required></div><div class="col-md-4 form-group d-flex gap-2"><input type="text" class="form-control" name="feature_description[]" placeholder="' +
    description +
